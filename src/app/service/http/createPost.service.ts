...
export class PostService {
  ...
  getPosts(pageNo: number, pageSize: number, sortBy: string, tags: string = ''): Observable<any> {
    const url = `${environment.apiUrl}/post/getAll`;

    // Set up query parameters for pagination, sorting, and tags
    const params = new HttpParams()
      .set('page_no', pageNo.toString())
      .set('page_size', pageSize.toString())
      .set('sort_by', sortBy)
      .set('tags', tags);  // Optional filtering by tags
